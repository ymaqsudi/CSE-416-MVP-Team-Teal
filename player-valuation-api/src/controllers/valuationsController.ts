...
import { leagueDraftFromQuery, valuationMapFor, parseMlbLeague, parseCategories, parseRosterSlots, parseNumTeams, parseBudget, filterPoolByLeague } from "../services/valuationContext.js";
import {
  undraftedPlayers,
  playerIdString,
  type PlayerLean,
...
export async function postValuationsBatch(req: Request, res: Response): Promise<void> {
  try {
    const ctx = await leagueDraftFromQuery(req.query.sessionId, { requireSession: true, mlbLeague: parseMlbLeague(req.query.mlbLeague), categories: parseCategories(req.query.categories), rosterSlotsPerTeam: parseRosterSlots(req.query.rosterSlots), numTeams: parseNumTeams(req.query.numTeams), budget: parseBudget(req.query.budget) });
    if (!ctx.ok) {
...
export async function getValuationsAll(req: Request, res: Response): Promise<void> {
  try {
    const ctx = await leagueDraftFromQuery(req.query.sessionId, { requireSession: false, mlbLeague: parseMlbLeague(req.query.mlbLeague), categories: parseCategories(req.query.categories), rosterSlotsPerTeam: parseRosterSlots(req.query.rosterSlots), numTeams: parseNumTeams(req.query.numTeams), budget: parseBudget(req.query.budget) });
    if (!ctx.ok) {
      res.status(ctx.status).json({ message: ctx.message });
      return;
    }
